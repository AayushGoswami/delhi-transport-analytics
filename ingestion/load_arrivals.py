...
#     print(f"\nReading stop_times from S3 (streaming)...")
#     s3 = boto3.client("s3")
#     response = s3.get_object(Bucket=S3_BUCKET, Key="stop_times.txt")
...
#             print(f"\nReached {MAX_ROWS} row limit.")
#             break

#         trip_id = row.get("trip_id", "").strip()
#         stop_id = row.get("stop_id", "").strip()
#         arrival_str = row.get("arrival_time", "").strip()
#         departure_str = row.get("departure_time", "").strip()
#         seq = row.get("stop_sequence", "0").strip()
...
#         departure_time = parse_gtfs_time(departure_str, SIMULATION_DATE)
#         delay = simulate_delay()
#         actual_time = scheduled_time + timedelta(seconds=delay)
#         route_id = trip_route_map.get(trip_id)
...
#             chunk_num += 1
#             batch = []
#             print(f"  Inserted {total_inserted} rows... (skipped {total_skipped})")
...
#     cur.execute("SELECT COUNT(*) FROM bus_arrivals")
#     print(f"  Verified count: {cur.fetchone()[0]}")
...
#     stats = cur.fetchone()
#     print(f"\nDelay statistics:")
#     print(f"  Total arrivals:  {stats[0]}")
#     print(f"  Delayed (>5min): {stats[1]}")
#     print(f"  Avg delay:       {stats[2]}s")
#     print(f"  Max delay:       {stats[3]}s")
